...
class S3Connector(BaseConnector):
    def __init__(
        ...
        self._bucket, self._prefix, self._extensions, self._region = (
            bucket,
            prefix,
            extensions or [".pdf"],
            region,
    ...
    async def list_uris(self) -> List[str]:
        try:
            ...
            return [
                ...
                if any(obj["Key"].endswith(e) for e in self._extensions)
            ]
        ...
        try:
            ...
            for uri in await self.list_uris():
                ...
                yield DiscoveredDocument(
                    ...
                    tenant_id=tenant_id,
                    metadata={"connector": "s3", "bucket": self._bucket},
                )
                os.unlink(local)
        except ImportError:
...
class AzureBlobConnector(BaseConnector):
    ...
    async def list_uris(self) -> List[str]:
        try:
            ...
            return [
                f"az://{self._container}/{b.name}"
                for b in client.get_container_client(self._container).list_blobs(
        ...
        try:
            ...
            for blob in cc.list_blobs(name_starts_with=self._prefix):
                ...
                with tempfile.NamedTemporaryFile(
                    suffix=Path(blob.name).suffix, delete=False
                ) as tmp:
                    tmp.write(
                        await asyncio.to_thread(
                            cc.get_blob_client(blob.name).download_blob().readall
                        )
                    )
                    local = tmp.name
                yield DiscoveredDocument(
                    source_uri=f"az://{self._container}/{blob.name}",
                    local_path=local,
