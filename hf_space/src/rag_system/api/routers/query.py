...
async def query_documents(request: Request, body: QueryRequest):
    ...
    try:
        ...
        if "413" in message or "payload too large" in lowered or "too large" in lowered:
            logger.warning("query_endpoint_context_too_large", error=message)
            return JSONResponse(
                status_code=413,
                content={
                    "detail": (
                        "The retrieved document context was too large for the AI provider. "
                        "Try a more specific question about one metric or section."
                    ),
                    "error_code": "context_too_large",
                },
            )
        logger.error("query_endpoint_failed", error=message)
