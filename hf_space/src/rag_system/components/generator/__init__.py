...
def _estimate_tokens(text: str) -> int:
    """Conservative token estimate (~3 chars/token) for provider limits."""
    return max(1, (len(text) + 2) // 3)


def _groq_limits(base_url: str, cfg) -> tuple[int, int, int]:
    """Groq dev tier enforces a hard ~8k token request ceiling."""
    if "groq.com" not in base_url.lower():
        return cfg.max_context_tokens, cfg.max_chars_per_chunk, cfg.max_tokens
    return (
        min(cfg.max_context_tokens, 4200),
        min(cfg.max_chars_per_chunk, 900),
        min(cfg.max_tokens, 1024),
    )
...
def _truncate_at_boundary(text: str, max_chars: int) -> str:
    ...
    for separator in (". ", ".\n", "\n"):
        ...
        if index >= body_limit // 3:
            return f"{snippet[: index + 1].rstrip()}…"
...
def fit_chunks_to_context_budget(
    ...
    *,
    ...
    """Trim ranked chunks so the LLM prompt stays within provider limits."""
    if not chunks:
        return [], False

    budget_tokens = max(256, max_context_tokens - reserved_tokens)
    ...
    for chunk in chunks:
        ...
        overhead = len(f"[Source: {chunk.source_document}{page}]\n") + 8
        remaining = budget_chars - used_chars - overhead
        if remaining < 160:
            was_trimmed = True
            break

        per_chunk_cap = min(max_chars_per_chunk, remaining)
        text = chunk.text
        if len(text) > per_chunk_cap:
            text = _truncate_at_boundary(text, per_chunk_cap)
            was_trimmed = True
...
def _prepare_generation_context(
    ...
    *,
    ...
    system_text = system_prompt or FINANCIAL_RAG_SYSTEM_PROMPT
    reserved_tokens = _estimate_tokens(f"{system_text}\nContext:\n\nQuestion: {query}") + 96
    fitted_chunks, was_trimmed = fit_chunks_to_context_budget(
        chunks,
        max_context_tokens=max_context_tokens,
        reserved_tokens=reserved_tokens,
        max_chars_per_chunk=max_chars_per_chunk,
    )
    context_block = _build_context_block(fitted_chunks)
    user_message = f"Context:\n{context_block}\n\nQuestion: {query}"
    return user_message, fitted_chunks, was_trimmed
...
class OpenAIGenerator(BaseGenerator):
    ...
    async def generate(
        ...
        effective_system = system_prompt or FINANCIAL_RAG_SYSTEM_PROMPT
        user_message, fitted_context, was_trimmed = _prepare_generation_context(
            context,
            query,
            effective_system,
            max_context_tokens=self._cfg.max_context_tokens,
            max_chars_per_chunk=self._cfg.max_chars_per_chunk,
        )
        if was_trimmed:
            logger.info(
                "llm_context_trimmed",
                ...
                used_chunks=len(fitted_context),
                max_context_tokens=self._cfg.max_context_tokens,
                tenant_id=tenant_id,
            )
...
class LocalVLLMGenerator(BaseGenerator):
    ...
    Config (Groq — LLM_CONFIG__PROVIDER=grok or groq):
        ...
        LLM_CONFIG__MODEL=openai/gpt-oss-120b
        GROQ_API_KEY=gsk-...
    ...
    async def generate(
        ...
        effective_system = system_prompt or FINANCIAL_RAG_SYSTEM_PROMPT
        context_budget, chars_per_chunk, max_output_tokens = _groq_limits(
            self._base_url, self._cfg
        )
            ...
            async with (
                ...
                for attempt in range(4):
                    user_message, fitted_context, was_trimmed = _prepare_generation_context(
                        ...
                        max_chars_per_chunk=chars_per_chunk,
                    )
                    if was_trimmed:
                        logger.info(
                            "llm_context_trimmed",
                    ...
                    if response.status_code == 413 and attempt < 3:
                        context_budget = max(1500, context_budget // 2)
                        chars_per_chunk = max(400, chars_per_chunk // 2)
                        max_output_tokens = max(256, max_output_tokens // 2)
                        logger.warning(
                            "llm_context_retry_after_413",
