...
class TestEntity:
    ...
    def test_with_value(self):
        e = Entity(id="m1", type="METRIC", name="Revenue", source_document="t.pdf", value="23.35B")
...
class TestInMemoryGraphStore:
    def test_add_entity(self):
        ...
        s.add_entity(
            Entity(id="e1", type="COMPANY", name="Tesla", source_document="d.pdf", tenant_id="acme")
        )
    ...
    def test_two_hop_neighbors(self):
        ...
        s.add_relation(
            Relation(
                subject_id="a", predicate="SUBSIDIARY_OF", object_id="b", source_document="d.pdf"
            )
        )
        s.add_relation(
            Relation(
                subject_id="b", predicate="SUBSIDIARY_OF", object_id="c", source_document="d.pdf"
            )
        )
    ...
    def test_predicate_filter(self):
        ...
        s.add_relation(
            Relation(
                subject_id="a", predicate="SUPPLIES_TO", object_id="c", source_document="d.pdf"
            )
        )
    ...
    def test_search_by_name_case_insensitive(self):
        ...
        s.add_entity(
            Entity(
                id="e1", type="COMPANY", name="Tesla Inc", source_document="d.pdf", tenant_id="t1"
            )
        )
    ...
    def test_stats(self):
        s = InMemoryGraphStore()
        s.add_entity(
            Entity(id="e1", type="COMPANY", name="A", source_document="d.pdf", tenant_id="acme")
        )
    ...
    def test_tenant_isolation(self):
        s = InMemoryGraphStore()
        s.add_entity(
            Entity(id="e1", type="COMPANY", name="A", source_document="d.pdf", tenant_id="a")
        )
        s.add_entity(
            Entity(id="e2", type="COMPANY", name="B", source_document="d.pdf", tenant_id="b")
        )
...
class TestGraphAugmentedRetriever:
    ...
    async def test_returns_base_with_populated_graph(self):
        s = InMemoryGraphStore()
        s.add_entity(
            Entity(id="e1", type="COMPANY", name="Tesla", source_document="d.pdf", tenant_id="t1")
        )
